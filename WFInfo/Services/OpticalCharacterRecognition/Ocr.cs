...
public partial class OCR : IOCR
{
    ...
    public async Task ProcessRewardScreen(Bitmap? file = null)
    {
        ...
        _firstChecks = _firstChecks
                       ...
                       .ToArray();
        if (_firstChecks.Length == 0 || CheckIfError())
        {
            ProcessingActive.GetAndSet(false);
            var end = Stopwatch.GetElapsedTime(start);
            Logger.Debug("----  Partial Processing Time, couldn't find rewards {Time}", end);
            ...
            if (_firstChecks.Length == 0)
                await _mediator.Publish(new ErrorDialogShow(time));
        }
    ...
    public string RewardScreenClipboard(
        in double platinum,
        string correctName,
        string plat,
        string? primeSetPlat,
    ...
    /// Using levenstein distance to check if the part name is close enough to a valid part name
    /// </summary>
    ...
    private bool PartNameValid(string partName)
    {
        ...
        var name = Main.DataBase.GetPartName(partName, out var levenDist, true, out var _);

        if (name.Length < minPartNameLength && _settings.Locale == "en")
            return false;

        //TODO: Add a min character for other locale here.
        if (_settings.Locale == "ko" && name.Replace(" ", string.Empty).Length < 6)
            return false;
        ...
        Point snapItOrigin)
    {
        ...
        var detectedParts = FindAllParts(snapItImageFiltered, snapItImage, rowHits, colHits);
        var end = Stopwatch.GetElapsedTime(start);
        ...
        if (detectedPartsCount != resultCount)
            Logger.Debug("Snap-it detected {Detected} parts, but only {Result} were valid", detectedPartsCount, resultCount);
        ...
        for (var i = 0; i < foundParts.Count; i++)
        {
            ...
            // TODO (rudzen) : Convert to request event?
            var primeSet = Main.DataBase.MarketData.GetValue(primeSetName) as JObject;
        ...
        await _mediator.Publish(SnapItOverloadDisposeImage.Instance);
        ...
        if (resultCount == 0)
        {
            ...
            await _mediator.Publish(new ErrorDialogShow(DateTime.UtcNow));
        }
        else
        {
    ...
    private static IEnumerable<TextWithBounds> GetTextWithBoundsFromImage(
        ...
        int rectYOffset)
    {
        ...
        do
        {
            ...
            currentWord = Re.Replace(currentWord, string.Empty);

            var currentWordSpan = currentWord.AsSpan().Trim();

            if (currentWordSpan.IsEmpty)
                continue;

            if (currentWordSpan.Length != currentWord.Length)
                currentWord = currentWordSpan.ToString();
            ...
            yield return new TextWithBounds(currentWord, bounds);
        ...
        int[] colHits)
    {
        ...
        {
            zones = _snapZoneDivider.DivideSnapZones(filteredImage, filteredImageClean, rowHits, colHits).ToArray();
            snapThreads = 4;
        }
        else
        {
            zones =
            [
                new SnapZone(filteredImageClean, new Rectangle(0, 0, filteredImageClean.Width, filteredImageClean.Height))
            ];
            snapThreads = 1;
        }
        ...
        for (var i = 0; i < snapThreads; i++)
        {
            ...
            {
                var taskResults = new List<TextWithBounds>(zones.Length);
                for (var j = tempI; j < zones.Length; j += snapThreads)
                {
                    //process images
        ...
        for (var threadNum = 0; threadNum < snapThreads; threadNum++)
        {
            ...
            foreach (var (currentWord, bounds) in snapTasks[threadNum].GetAwaiter().GetResult())
            {
                //word is valid start comparing to others
                ...
                using (var g = Graphics.FromImage(filteredImage))
                {
                    ...
                    {
                        //Determine whether the box is too large, false positives in OCR can scan items (such as neuroptics, chassis or systems) as a character(s).
                    ...
                    var withSpaces = AddSpacesToSentence(currentWord);
                    if (withSpaces != currentWord)
                        Logger.Debug("Added spaces to {Word} to make {WithSpaces}", currentWord, withSpaces);

                    g.DrawString(currentWord, Font, Brushes.Pink, new Point(paddedBounds.X, paddedBounds.Y));

                    string AddSpacesToSentence(ReadOnlySpan<char> text)
                    {
                        if (text.IsEmpty)
                            return string.Empty;
        ...
        foreach (var itemGroup in foundItems.Where(itemGroup => itemGroup.Items.Count != 0))
        {
            if (itemGroup.Items.Count == 1)
            {
                results.Add(new InventoryItem(itemGroup.Items[0].Name.Trim(), itemGroup.Rectangle));
            }
            else
            {
        ...
        Font font)
    {
        ...
        bool itemRemoved;
        for (var i = 0; i < foundItemsBottom.Count; i += (!itemRemoved).AsByte())
        {
            itemRemoved = false;
    ...
    public async Task ProcessProfileScreen(Bitmap fullShot)
    {
        ...
        foreach (var partName in foundParts.Select(part => part.Name).Where(partName => PartNameValid($"{partName} Blueprint")))
        {
        ...
        using (var g = Graphics.FromImage(profileImage))
        {
            ...
            for (var y = 0; y < profileImageClean.Height - 1; y = nextYCounter == 0 ? nextY : y + 1)
            {
                for (var x = 0; x < imgWidth; x += probeInterval) //probe every few pixels for performance
                {
                    ...
                    var hitRatio = (double)(hits) / areaWidth;
                    ...
                    if (ratioChanges != 4 || width < 2.4 * height || width > pixelWidth * height)
                    {
    ...
    public unsafe Bitmap ScaleUpAndFilter(Bitmap image, WFtheme active, out int[] rowHits, out int[] colHits)
    {
        ...
        for (var i = 0; i < numBytes; i += pixelSize)
        {
            ...
            {
                colorBuffer.Fill(byte.MaxValue);
            }
        }
        ...
        Bitmap fullScreen)
    {
        ...
        for (var i = 0; i < topFive.Length; i++)
        {
            ...
            Logger.Debug("RANK {Rank} SCALE: {TopFive}%\t\t{PercW} -- {TopW}, {MidW}, {BotW}",
                5 - i,
                tFive + 50,
                percWeights[tFive].ToString("F2", ApplicationConstants.Culture),
                topWeights[tFive].ToString("F2", ApplicationConstants.Culture),
                midWeights[tFive].ToString("F2", ApplicationConstants.Culture),
                botWeights[tFive].ToString("F2", ApplicationConstants.Culture));
        }
    ...
    [SkipLocalsInit]
    private unsafe List<Bitmap> FilterAndSeparatePartsFromPartBox(Bitmap partBox, in WFtheme active)
    {
        const int pixelSize = 4;
        ...
        //ARGB, order in array is BGRA

        // Grab the image (new filtered) data and lock it
        var lockedFilteredData = filtered.LockBits(
            rect: new Rectangle(0, 0, width, height),
            flags: ImageLockMode.WriteOnly,
            format: filtered.PixelFormat
        );
        var filteredHeight = Math.Abs(lockedFilteredData.Stride) * height;
        var filteredBytes = new Span<byte>(lockedFilteredData.Scan0.ToPointer(), filteredHeight);

        // Grab the part box data and lock it
        ...
        for (var x = 0; x < width; x++)
        {
            ...
            count = Math.Min(count, height / 3);
            var sinVal = Math.Cos(8 * x * Math.PI / width);
    ...
    private static int GetFinalPartBoxHeight(in int height, ReadOnlySpan<int> counts)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static bool IsRectangleWithinImageBounds(Image image, in Rectangle rectangle)
    {
        // Check if the rectangle's X coordinate is within the image's width
        if (rectangle.X < 0 || rectangle.X + rectangle.Width > image.Width)
            return false;
...
public struct InventoryItem(string itemName, Rectangle boundingBox, bool showWarning = false)
{
    ...
    public Rectangle Bounding { get; } = boundingBox;
