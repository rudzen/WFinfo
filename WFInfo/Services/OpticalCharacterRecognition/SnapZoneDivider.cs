...
public sealed class SnapZoneDivider(ApplicationSettings settings) : ISnapZoneDivider
{
    ...
    public SnapZone[] DivideSnapZones(
        ...
        int[] colHits)
    {
        ...
        var zones = DivideToTextBlocks(
            filteredImageClean: filteredImageClean,
            rows: CollectionsMarshal.AsSpan(rows),
            cols: cols,
            rowHeight: rowHeight
        );
    ...
    private static List<Row> CombineRows(
        Image filteredImage,
    ...
    private static SnapZone[] DivideToTextBlocks(
        Bitmap filteredImageClean,
        ReadOnlySpan<Row> rows,
        ReadOnlySpan<Column> cols,
        int rowHeight)
    {
        var zones = new SnapZone[rows.Length * cols.Length];
        var zone = 0;

        ref var rowRef = ref MemoryMarshal.GetReference(rows);
        ref var colRef = ref MemoryMarshal.GetReference(cols);
    ...
    private static void DrawRectangles(
        Bitmap filteredImage,
        ReadOnlySpan<SnapZone> zones,
        int rowHeight)
    {
        ref var zoneRef = ref MemoryMarshal.GetReference(zones);
