...
using WFInfo.Models.WarframeMarket;
    ...
    IRequestHandler<DataRequest, DataResponse>
{
    ...
    private readonly WebSocket _marketSocket = new("wss://warframe.market/socket?platform=pc");
        ...
        IOCR ocr)
    {
        ...
        _marketSocket.SslConfiguration.EnabledSslProtocols = SslProtocols.None;
    }
    ...
    public async Task ReloadItems()
    {
        ...
        try
        {
            ...
            var witems = System.Text.Json.JsonSerializer.Deserialize<WarframeMarketItems>(body);
        ...
        {
            Logger.Error(exception, "Failed to get items from warframe.market");
        }
    ...
    {
        if (_autoThread is { IsCompleted: false })
            return;
        ...
        {
            _autoThread = Task.Run(AutoTriggered);
        ...
        Task.Run(async () =>
        {
            ...
            Logger.Debug("AutoList: {List}, AutoCSV: {Csv}, AutoCount: {Count}", _settings.AutoList, _settings.AutoCSV, _settings.AutoCount);
            ...
            foreach (var rewardScreen in Main.ListingHelper.PrimeRewards)
            {
                ...
                if (_settings.AutoCSV)
                {
                    ...
                    csv.Append(DateTime.UtcNow.ToString("yyyy-MM-dd HH-mm-ssff", ApplicationConstants.Culture));
                    csv.Append(',');
                    csv.Append(Main.ListingHelper.SelectedRewardIndex);
                    for (var i = 0; i < 4; i++)
                    {
                        ...
                        {
                            csv.Append(",\"\",0,0"); //fill empty slots with "",0,0
                        }
                    }
            ...
            if (_settings.AutoCSV)
            {
                ...
                await File
                      .AppendAllTextAsync(Path.Combine(ApplicationConstants.AppPath, "rewardExport.csv"), csv.ToString())
    ...
    public async Task GetUserLogin(string email, string password)
    {
        ...
        Logger.Debug("Response: {Response}", censoredResponse);
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        if (_marketSocket.IsAlive)
            return false;

        _marketSocket.OnMessage += (_, e) =>
        {
        ...
        _marketSocket.OnMessage += (sender, e) =>
        {
        ...
        _marketSocket.OnOpen += (sender, e) =>
        {
            var isRunning = _process.IsRunning();
            _marketSocket.Send(isRunning && _process is { GameIsStreamed: false }
        ...
        try
        {
            _marketSocket.SetCookie(new WebSocketSharp.Net.Cookie("JWT", _encryptedDataService.JWT));
            _marketSocket.ConnectAsync();
        }
        catch (Exception e)
        {
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    ...
    {
        if (_marketSocket.ReadyState != WebSocketState.Open)
            return;
    ...
    public async Task<bool> IsJWTvalid()
    {
        ...
        try
        {
            ...
            // remove the code that can compromise an account.
            profile["profile"]["check_code"] = "REDACTED";
    ...
    {
        Logger.Debug("Checking websocket alive status. status={Status},at={At}", _marketSocket.IsAlive, request.RequestedAt);
        return new ValueTask<WebSocketAliveStatusResponse>(new WebSocketAliveStatusResponse(_marketSocket.IsAlive));
    }
